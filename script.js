...
const savingInput = document.querySelector('.saving_input');
...
const savcard = document.querySelector('.savings_card');
...
function btnEvents() {
    ...
    const savingsBtn = document.querySelector('#btn_savings');
...
function savingsFun() {
    const savingValue = Number(savingInput.value);
    if (savingValue <= 0 || isNaN(savingValue)) {
        alert("Please enter a value greater than 0");
    } 
    else if(savingValue > budcard.textContent){
        alert("Please enter a saving value less than budget");

    }
        else {
        savcard.textContent = savingInput.value;
        savingInput.value = "";
        showbal();
...
function showbal() {
    let bal = parseInt(budcard.textContent) - parseInt(expcard.textContent)-parseInt(savcard.textContent);
